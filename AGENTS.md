...
Tasks are tracked in `.tasks/items/*.md` — individual markdown files with YAML frontmatter.

- `.tasks/README.md` has the full schema reference
- `status`: todo | in_progress | review | done | blocked
- `assignee`: null | "user" | "<agent-name>"
- `needs_guidance: true` when an agent is blocked and needs user input
- Body sections: `## Description`, `## Acceptance Criteria`, `## Progress`, `## Blockers`, `## Notes`
- **Acceptance criteria are generated by agents** — the user writes only a brief description
- When specing a task, the team leader reads relevant source files, calls @designer directly for exercise design, passes output to the @general spec agent, which follows the new-exercise skill and writes concrete ACs
- When working on a task, write progress to the `## Progress` section of its task file
- When blocked, set `needs_guidance: true` and describe the blocker in `## Blockers`
- Return short status summaries (1-3 lines) to the spawning agent — do not return full transcripts
- Task files are the durable state; read them again when resuming, don't rely on previous agent context

## Parallel work with worktrees

Multiple agents work in parallel via git worktrees in `.worktrees/` (gitignored).

- Each worktree is a separate checkout on its own branch
- Each worktree has a symlinked `node_modules` from the root
- Agents open OpenCode with `--dir .worktrees/agent-NNN/`
- The user tests in the main checkout while agents implement
- Task file updates are committed to feature branches and merged to main when ready
