<script lang="ts">
  ...
  let userLatex = $derived(
    mode === 'fraction' ? `\\dfrac{${userNumLatex}}{${userDenLatex}}` : userNumLatex,
  );

  let correctNumParts = $derived(
    mode === 'fraction'
      ? (exercise.answer.split(';')[0]?.split(',').map((s) => s.trim()) ?? [])
      : exercise.answer.split(',').map((s) => s.trim()),
  );
  let correctDenParts = $derived(
    mode === 'fraction' ? (exercise.answer.split(';')[1]?.split(',').map((s) => s.trim()) ?? []) : [],
  );
  ...
  let correctDenLatex = $derived(
    mode === 'fraction' ? formatCollectingAnswer(correctDenParts, denVariableParts) : '',
  );
...
<ExerciseShell {exercise} {feedback} submitAnswer={submitAnswer} {onNext} {validationError}>
  {#snippet submitExtra()}
    {#if feedback === null}
